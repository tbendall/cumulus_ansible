...
URL_TMPL = "https://{device}:8765/nvue_v1/evpn/access-vlan-info"
...
if __name__ == "__main__":
    ...
    for d, r in results.items():
        if r.get("ok") and isinstance(r.get("data"), dict):
            for vlan in r['data']['vlan']:
                
                vlan_dict = r['data']['vlan'][vlan]

                print(f"{vlan} member interfaces: {list(vlan_dict['member-interface'])}")
